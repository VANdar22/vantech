...
const TargetCursor = ({
  ...
  const isMobile = useMemo(() => {
    // Skip mobile detection if we're on the server-side
  ...
  const handleMouseMove = useCallback((e) => {
    ...
    // If we have a container ref, check if we're inside it
    if (containerRef?.current) {
      ...
      // Only show custom cursor when inside the container
    ...
    moveCursor(x, y);
  }, [isMobile, moveCursor, containerRef, hideDefaultCursor]);
  ...
  useEffect(() => {
    ...
    // Only hide cursor by default if we're not using a container
    // or if we want to hide it globally
